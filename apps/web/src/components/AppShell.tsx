      ...
      <header className="sticky top-0 z-40 bg-bg/90 backdrop-blur border-b border-line">
        <div className="max-w-7xl mx-auto px-4 h-12 flex items-center gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1 text-[13px]">
            {NAV.map(([href, key]) => (
              <Link key={href} href={href} className={`px-2 py-1 rounded-md ${path === href || (href !== '/' && path.startsWith(href)) ? 'bg-accent-soft text-accent' : 'text-ink-2 hover:text-ink'}`}>{t(key)}</Link>
            ))}
          </nav>
          ...
          <LocaleSwitcher />
          <button className="flex items-center gap-2" title={t('signOut', { email: me.user.email })} onClick={async () => { await authClient.signOut(); router.replace('/login'); }}>
        ...
        <nav className="md:hidden flex overflow-x-auto gap-1 px-3 pb-2 text-[13px]">
          {NAV.map(([href, key]) => <Link key={href} href={href} className={`px-2 py-0.5 rounded-md whitespace-nowrap ${path === href ? 'bg-accent-soft text-accent' : 'text-ink-2'}`}>{t(key)}</Link>)}
        </nav>
      </header>
      <ProcessingBanner />
...
function ProcessingBanner() {
  ...
  if (!n) return null;
  return <div className="max-w-7xl mx-auto px-4 pt-3"><div className="banner">{t('analyzing', { count: n })}</div></div>;
}
