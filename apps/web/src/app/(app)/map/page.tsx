...
export default function MapPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="banner">The interactive map arrives in Phase 3. This is the list of event pins it will show.</div>
      {pins.length === 0 && !q.isLoading && <p className="text-ink-3">No events with a location yet.</p>}
      <ul className="card divide-y divide-line">
        {pins.map((p) => (
          <li key={p.id} className="p-3 flex items-center justify-between gap-3 text-[13px]">
            ...
            <a className="text-accent" target="_blank" rel="noreferrer" href={`https://www.openstreetmap.org/?mlat=${p.lat}&mlon=${p.lon}#map=15/${p.lat}/${p.lon}`}>OSM ↗</a>
          </li>
        ))}
      </ul>
    </div>
